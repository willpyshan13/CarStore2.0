...
docker run -d -v /etc/localtime:/etc/localtime:ro -v /var/local/java-log/gateway:/var/local/java-log/gateway --memory 1000m --restart=always --name car-gateway-dev --network host car-gateway-dev
exit
remotessh
