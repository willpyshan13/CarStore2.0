...
docker rmi car-utility-pro
docker build -t car-utility-pro -f ../src/main/docker/prod/Dockerfile .

ssh root@58.33.180.67 << remotessh
rm -rf /var/local/docker-images-tar/car-utility-pro.tar
docker save car-utility-pro -o /var/local/docker-images-tar/car-utility-pro.tar
scp /var/local/docker-images-tar/car-utility-pro.tar root@8.136.111.199:/var/local/docker-images-tar/car-utility-pro.tar
exit
remotessh
ssh root@8.136.111.199 << remotessh
docker stop car-utility-pro
docker rm car-utility-pro
docker rmi car-utility-pro
docker load < /var/local/docker-images-tar/car-utility-pro.tar
docker run -d -v /etc/localtime:/etc/localtime:ro -v /var/local/java-log/utility:/var/local/java-log/utility --restart=always --name car-utility-pro --network host car-utility-pro
exit
remotessh
