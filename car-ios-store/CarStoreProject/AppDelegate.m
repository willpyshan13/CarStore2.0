...
//        NSLog(@"stepstep====%@, success==%u, errorInfo==%@,suggestion=== %@", @(step), result.success, result.errorInfo, result.suggestion);
//    }];
...
-(void)configMainController
{
    ...
    {
        NSString *checkSts=SafeValue([NSString stringWithFormat:@"%@",model.checkSts]);
        DLog(@"checkSts==%@",checkSts);

        if ([checkSts isEqualToString:@"1"]) {
...
//            }
            ...
            NSLog(@"城市----%@",placeMark.locality);//locality
            NSLog(@"subLocality==%@",placeMark.subLocality);//当前的位置
