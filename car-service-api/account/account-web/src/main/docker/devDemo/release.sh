...
docker rmi car-account-dev-demo
docker build -t car-account-dev-demo -f ../src/main/docker/devDemo/Dockerfile .

ssh root@58.33.180.67 << remotessh
rm -rf /var/local/docker-images-tar/car-account-dev-demo.tar
docker save car-account-dev-demo -o /var/local/docker-images-tar/car-account-dev-demo.tar
scp /var/local/docker-images-tar/car-account-dev-demo.tar root@8.136.105.20:/var/local/docker-images-tar/car-account-dev-demo.tar
exit
remotessh
ssh root@8.136.105.20 << remotessh
docker stop car-account-dev-demo
docker rm car-account-dev-demo
docker rmi car-account-dev-demo
docker load < /var/local/docker-images-tar/car-account-dev-demo.tar
docker run -d -v /etc/localtime:/etc/localtime:ro -v /var/local/java-log/accountDemo:/var/local/java-log/account --memory 1000m --restart=always --name car-account-dev-demo --network host car-account-dev-demo
exit
remotessh
