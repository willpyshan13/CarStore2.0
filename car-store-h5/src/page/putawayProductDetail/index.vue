...
<template>
  <div class="putawayProduct">
    ...
    <div class="cont">
      <ul>
        <li>
          <van-cell value-class="disabled-color" title="一级分类" :value="one" />
          <van-action-sheet
            v-model="show"
            :actions="actions"
            @select="onSelect"
        ...
        <li>
          <van-cell value-class="disabled-color" title="二级分类" :value="two" />
          <van-action-sheet
            v-model="show2"
            :actions="actions2"
            @select="onSelect2"
        ...
        <blockquote v-if="$route.query && $route.query.oneId == '1007'">
          <li>
            <van-cell
              is-link
              title="请选择品牌"
        ...
        <li v-if="oneUuid == '1002'">
					<van-cell is-link title="请填写轮胎型号" :value="tyreNo" @click="tireModelClick()" />
        ...
        <li style="height: auto">
          ...
          >
            <pull-down
              v-if="$route.query && $route.query.oneId == '1006'"
              :title="txt + '名称'"
              selectType="share_station_type"
              @baseVal="(v) => { item.name = v }"
            ></pull-down>
            ...
            <pull-down
              v-if="$route.query && $route.query.oneId == '1006'"
              :title="txt + '价格'"
              selectType="share_station_charging_money"
              @baseVal="(v) => { item.actAmt = v }"
            ></pull-down>
            <div class="item" v-else>
            ...
            <div v-if="oneUuid == '1005'">
						...
						<div v-if="oneUuid == '1006'">
							...
							<div class="item">
								<div class="div_l time">最近一次维护时间</div>
								<div class="div_r">
									<van-field is-link v-model="item.bak3" placeholder="请选择最近一次维护时间" @click="showPopup(index)" />
								</div>
            ...
            <div style="width: 100%; text-aline: center">
              <van-button
                ...
                >删除当前{{ txt }}信息</van-button
              >
            </div>
            <div class="baseTitle">
              <div
                v-if="index + 1 == dynamicItem.length"
                @click="addItem"
                class="addWuliao"
              >
                +添加{{ txt }}
        ...
        <li>
          <div class="item">
            ...
            <div class="div_r">
              <van-field
              	:disabled="fuwuDisabled"
                v-model="fuwu"
                type="number"
                placeholder="请填写价格"
                 @focus="fuwuFocus" @blur="fuwuFocus"
              />
            </div>
            <div>(元)</div>
          </div>
...
export default {
  ...
  created() {
    this.setTxt(this.$route.query && this.$route.query.oneId)
    this.dynamicItem = [
      {
        actAmt: "",
        name: "",
  ...
  methods: {
  	...
  	queryStoreDetailByUser () {
			...
			confirmTireModel (val) {
				this.tyreNo = val[0] + val[1] + val[2] + '/' + val[3] + val[4] + ' ' + val[5] + ' ' + val[6] + val[7] + val[8]
  	...
  	// 选择最近一次维修时间
			...
			confirmPicker(val) {
				...
				console.log(this.dynamicItem[this.timeIndex].recentMaintenanceTime, 'timeValue');
			},
    setTxt(code) {
      if (code == '1005') {
        this.txt = '技师'
    ...
    getMoney() {
      managerApi.zidian('goods_service_money').then((res) => {
        if (res.code == "0000") {
//        const resData = res.data;
//        const obj = resData.filter(x => this.$route.query && x.lableCode == this.$route.query.oneId)[0] || {}''
          // this.platformServiceMoney = obj.lableValue
        }
      });
    },
    parentFnUuid(id, v) {
    ...
    getDetail() {
      ...
      accountApi.getDetail(params).then((res) => {
        if (res.code == "0000") {
          ...
          this.setTxt(res.data.levelOneUuid)
...
//              storeUuid: "6d0ef7bbeaec4fbc9c82ce2ac7d4628b",
