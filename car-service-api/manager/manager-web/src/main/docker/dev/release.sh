...
docker run -d -v /etc/localtime:/etc/localtime:ro -v /var/local/java-log/manager:/var/local/java-log/manager --memory 1100m --restart=always --name car-manager-dev --network host car-manager-dev
exit
remotessh
