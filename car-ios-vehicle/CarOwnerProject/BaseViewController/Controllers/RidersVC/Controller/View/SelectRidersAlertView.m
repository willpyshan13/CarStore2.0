...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.type==3) {
        QueryTreeListModel *model = self.couponArr[indexPath.row];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        QueryTreeListModel *model = self.couponArr[indexPath.row];
        if (self.delegate && [self.delegate respondsToSelector:@selector(getSelectQueryTreeList:)]) {
               [self.delegate getSelectQueryTreeList:model];
        }
