...
docker run -d -v /etc/localtime:/etc/localtime:ro -v /etc/timezone:/etc/timezone:ro -v /var/local/java-log/order:/var/local/java-log/order --memory 1000m --restart=always --name car-order-dev --network host car-order-dev
exit
remotessh
