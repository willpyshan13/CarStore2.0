...
docker rmi car-manager-dev-demo
docker build -t car-manager-dev-demo -f ../src/main/docker/devDemo/Dockerfile .

ssh root@58.33.180.67 << remotessh
rm -rf /var/local/docker-images-tar/car-manager-dev-demo.tar
docker save car-manager-dev-demo -o /var/local/docker-images-tar/car-manager-dev-demo.tar
scp /var/local/docker-images-tar/car-manager-dev-demo.tar root@8.136.105.20:/var/local/docker-images-tar/car-manager-dev-demo.tar
exit
remotessh
ssh root@8.136.105.20 << remotessh
docker stop car-manager-dev-demo
docker rm car-manager-dev-demo
docker rmi car-manager-dev-demo
docker load < /var/local/docker-images-tar/car-manager-dev-demo.tar
docker run -d -v /etc/localtime:/etc/localtime:ro -v /var/local/java-log/managerDemo:/var/local/java-log/manager --memory 1100m --restart=always --name car-manager-dev-demo --network host car-manager-dev-demo
exit
remotessh
