...
docker rmi car-order-pro
docker build -t car-order-pro -f ../src/main/docker/prod/Dockerfile .

ssh root@58.33.180.67 << remotessh
rm -rf /var/local/docker-images-tar/car-order-pro.tar
docker save car-order-pro -o /var/local/docker-images-tar/car-order-pro.tar
scp /var/local/docker-images-tar/car-order-pro.tar root@8.136.111.199:/var/local/docker-images-tar/car-order-pro.tar
exit
remotessh
ssh root@8.136.111.199 << remotessh
docker stop car-order-pro
docker rm car-order-pro
docker rmi car-order-pro
docker load < /var/local/docker-images-tar/car-order-pro.tar
docker run -d -v /etc/localtime:/etc/localtime:ro -v /var/local/java-log/order:/var/local/java-log/order --restart=always --name car-order-pro --network host car-order-pro
exit
remotessh
